...
Setup Test Data
    ${language_id} =        API Create language        ${PRIMARY_LANGUAGE}
    Set Suite Variable      ${language_id}
...
Validate primary contact language record is created and updated in contact
    ...
    ${contact_id}=                    API Create Contact     hed__Primary_Language__c=${language_id}
    ${language_name}=                 Get Language Field Value  Contact  ${contact_id}  Primary_Language__c
    Should Be Equal As Strings        ${language_name}      ${PRIMARY_LANGUAGE}
    &{contact_language}=              Get Contact Language Value  Contact_Language__c  ${contact_id}
