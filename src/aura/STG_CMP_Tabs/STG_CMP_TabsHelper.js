({
    ...
    getHierarchySettings: function (component) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                var settingsNoPrefix = this.removePrefixHierarchySettings(
                    settings,
                    prefix
                );
                ...
                component.set(
                    "v.householdRecTypeId",
                    settingsNoPrefix.Household_Addresses_RecType__c
                );
                component.set(
                    "v.adminAccRecTypeId",
                    settingsNoPrefix.Administrative_Account_Record_Type__c
                );
                ...
                component.set(
                    "v.affiliationRoleMapValue",
                    settingsNoPrefix.Affl_ProgEnroll_Role_Map__c
                );
                component.set(
                    "v.affiliationStatusMapValue",
                    settingsNoPrefix.Affl_ProgEnroll_Status_Map__c
                );
                component.set(
                    "v.affiliationStatusDeleteMapValue",
                    settingsNoPrefix.Affl_ProgEnroll_Del_Status__c
                );
                component.set(
                    "v.defaultContactLanguageFluencyValue",
                    settingsNoPrefix.Default_Contact_Language_Fluency__c
                );
                component.set(
                    "v.defaultReciprocalMethodValue",
                    settingsNoPrefix.Reciprocal_Method__c
                );
                component.set(
                    "v.defaultPreferredPhoneValue",
                    settingsNoPrefix.Preferred_Phone_Selection__c
                );
                component.set(
                    "v.adminNameFormat",
                    settingsNoPrefix.Admin_Account_Naming_Format__c
                );
                component.set(
                    "v.hhNameFormat",
                    settingsNoPrefix.Household_Account_Naming_Format__c
                );
                component.set(
                    "v.adminOtherDisplay",
                    this.getOtherDisplay(
                        component,
                        settingsNoPrefix.Admin_Account_Naming_Format__c
                    )
                );
                ...
                this.getAccountRecordTypes(
                    component,
                    settingsNoPrefix.Accounts_to_Delete__c,
                    settingsNoPrefix.Accounts_Addresses_Enabled__c,
                    settingsNoPrefix.Account_Processor__c,
                    settingsNoPrefix.Household_Addresses_RecType__c,
                    settingsNoPrefix.Administrative_Account_Record_Type__c
                );
                // Get Affiliation Role Picklist
                this.getAffiliationRolePicklistEntries(
                    component,
                    settingsNoPrefix.Affl_ProgEnroll_Role_Map__c
                );
                // Get Affiliation Status Picklist
                this.getAffiliationStatusPicklistEntries(
                    component,
                    settingsNoPrefix.Affl_ProgEnroll_Status_Map__c,
                    settingsNoPrefix.Affl_ProgEnroll_Del_Status__c
                );
                // Get Course Connection Record Types
                this.getCourseConnectionRecordTypes(
                    component,
                    settingsNoPrefix.Student_RecType__c,
                    settingsNoPrefix.Faculty_RecType__c
                );
                // Get Contact Language Fluency Picklist
                this.getFluencyPicklistEntries(
                    component,
                    settingsNoPrefix.Default_Contact_Language_Fluency__c
                );
                // Get Preferrd Phone Picklists
                this.getPreferredPhonePicklistEntries(
                    component,
                    settingsNoPrefix.Preferred_Phone_Selection__c
                );
                // Get ReciprocalMethod Picklist Entries
                this.getReciprocalMethodOptions(
                    component,
                    settingsNoPrefix.Reciprocal_Method__c
                );
    ...
    getAccountRecordTypes: function (
        component,
        accsToDelete,
        accsMultiAddr,
        accTypeId,
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                var accTypesToDeleteSelected = this.getRecTypesSelected(
                    component,
                    accsToDelete,
                    accRecTypes
                );
                ...
                var accTypesAddrSelected = this.getRecTypesSelected(
                    component,
                    accsMultiAddr,
                    accRecTypes
                );
            ...
            if (response.getState() === "SUCCESS") {
                ...
                for (var property in picklistValues) {
                    if (picklistValues.hasOwnProperty(property)) {
                        affiliationRolePicklistEntries.push({
                            picklistLabel: property,
                            picklistValue: picklistValues[property]
                        });
                ...
                component.set(
                    "v.affiliationRolePicklistEntries",
                    affiliationRolePicklistEntries
                );
            } else if (response.getState() === "ERROR") {
                this.displayError(response);
            }
    ...
    getAffiliationStatusPicklistEntries: function (component, statusValue, deleteValue) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                for (var property in picklistValues) {
                    if (picklistValues.hasOwnProperty(property)) {
                        affiliationStatusPicklistEntries.push({
                            picklistLabel: property,
                            picklistValue: picklistValues[property]
                        });
                ...
                component.set(
                    "v.affiliationStatusPicklistEntries",
                    affiliationStatusPicklistEntries
                );
            } else if (response.getState() === "ERROR") {
                this.displayError(response);
            }
    ...
    getCourseConnectionRecordTypes: function (component, studentTypeId, facultyTypeId) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                for (var property in recTypesObj) {
                    if (recTypesObj.hasOwnProperty(property)) {
                        courseConnectionRecTypes.push({
                            name: property,
                            id: recTypesObj[property]
                        });
                ...
                for (var property in picklistValues) {
                    if (picklistValues.hasOwnProperty(property)) {
                        ...
                        if (property.indexOf(fluencyValue) > -1) {
                            component.set(
                                "v.defaultContactLanguageFluencyLabel",
                                property
                            );
                        }
                    }
    ...
    getPreferredPhonePicklistEntries: function (component, preferredValue) {
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                for (var property in picklistValues) {
                    if (
                        picklistValues.hasOwnProperty(property) &&
                        property != "Business Phone"
                    ) {
                        preferredPhonePicklistEntries.push({
                            picklistLabel: property,
                            picklistValue: property
                        });

                        if (
                            preferredValue == undefined ||
                            (preferredValue != undefined &&
                                property.indexOf(preferredValue)) > -1
                        ) {
                ...
                component.set(
                    "v.preferredPhonePicklistEntries",
                    preferredPhonePicklistEntries
                );
            } else if (response.getState() === "ERROR") {
                this.displayError(response);
            }
    ...
    getReciprocalMethodOptions: function (component, reciprocalMethodValue) {
        ...
        reciprocalMethodOptions.push({
            picklistLabel: $A.get("$Label.c.stgReciprocalMethodListSetting"),
            picklistValue: "List Setting"
        });
        reciprocalMethodOptions.push({
            picklistLabel: $A.get("$Label.c.stgReciprocalMethodValueInversion"),
            picklistValue: "Value Inversion"
        });

        // Load default Reciprocal Label (List Setting)
        if (reciprocalMethodValue == "List Setting") {
            component.set(
                "v.defaultReciprocalMethodLabel",
    ...
    saveSettings: function (component) {
        ...
        var hierarchySettings = this.addPrefixHierarchySettings(
            component.get("v.hierarchySettings"),
            prefix
        );
    ...
    processTabs: function (component, btnId) {
        ...
        let tabNav = component
            .find("tabs")
            .getElement()
            .querySelector("ul.slds-tabs--default__nav");
        ...
        matches.forEach(function (element) {
            ...
            let selectedTab =
                element.dataset.auraRenderedBy == tabItem.getGlobalId() || false;
