({
    ...
    removePrefixListSettings: function (settings, namespacePrefix) {
        return this.processPrefixListSettings(
            settings,
            namespacePrefix,
            this.removePrefix
        );
    ...
    processPrefixListSettings: function (settings, namespacePrefix, prefixHandlingFunc) {
        if (namespacePrefix && namespacePrefix.length > 0) {
            ...
            for (var key in settings) {
                //Iterate over each row - key is just a numeric index here
                if (settings.hasOwnProperty(key)) {
                    var obj = {}; //create object
                    for (var key2 in settings[key]) {
                        //Iterate over each field in each row - key2 is the actual field name
    ...
    addPrefixHierarchySettings: function (settings, namespacePrefix) {
        return this.processPrefixHierarchySettings(
            settings,
            namespacePrefix,
            this.addPrefix
        );
    ...
    processPrefixHierarchySettings: function (
        settings,
        namespacePrefix,
        prefixHandlingFunc
    ) {
    ...
    displayError: function (response) {
        ...
        if (
            errors &&
            errors[0].message &&
            typeof errors[0].pageErrors == "undefined" &&
            typeof errors[0].fieldErrors == "undefined"
        ) {
            ...
            errors[0].fieldErrors.Name[0] &&
            errors[0].fieldErrors.Name[0].message
        ) {
            throw new Error("Error message: " + errors[0].fieldErrors.Name[0].message);
        } else if (
            errors &&
            errors[0].fieldErrors &&
            errors[0].fieldErrors.Id &&
            errors[0].fieldErrors.Id[0] &&
            errors[0].fieldErrors.Id[0].message
        ) {
