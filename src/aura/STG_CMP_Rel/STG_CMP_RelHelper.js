({
    ...
    loadRelAutoCreateSettings: function (component) {
        ...
        autoCreateSettingsAction.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS") {
                ...
                if (settings.length === 0) {
                    component.set(
                        "v.noAutoCreateSettings",
                        $A.get("$Label.c.noAutoCreateSettings")
                    );
                }
                component.set(
                    "v.autoCreateSettings",
                    this.removePrefixListSettings(settings, prefix)
                );
            } else if (response.getState() === "ERROR") {
                this.displayError(response);
    ...
    saveRelSettings: function (component) {
        ...
        var reciprocalSettings = this.addPrefixListSettings(
            component.get("v.reciprocalSettings"),
            prefix
        );
        this.callServerVoidMethod(saveRecSettingsAction, {
            reciprocalSettings: reciprocalSettings
        });

        var saveAutoCreateSettingsAction = component.get("c.saveAutoCreateSettings");
        var autoCreateSettings = this.addPrefixListSettings(
            component.get("v.autoCreateSettings"),
            prefix
        );
        this.callServerVoidMethod(saveAutoCreateSettingsAction, {
    ...
    deleteAutoCreateRow: function (component, id, position) {
        this.deleteRow(
            component,
            "c.deleteAutoCreateRecord",
            "v.autoCreateSettings",
            id,
            position,
            "v.noAutoCreateSettings",
            "noAutoCreateSettings",
            component.get("v.namespacePrefix")
        );
    },
